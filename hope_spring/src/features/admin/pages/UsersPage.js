    ...
    const fetchUsers = async () => {
      try {
        ...
        const mapped = res.data.map((u) => ({
          ...
          status: "Active", // Default for now (no field in DB)
        }));
        setUsers(mapped);
      } catch (err) {
      ...
      <div className="min-h-screen bg-gradient-to-b from-[#f7f5fb] to-[#f3f0fa] p-6 md:p-10 space-y-10">
        ...
        <div>
          ...
          <p className="text-gray-600 mt-1">
            Overview of all users currently registered in the HopeSpring system.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          ...
          <div className="bg-white rounded-2xl shadow-md p-6 flex items-center justify-between">
            <div>
              ...
              <h2 className="text-2xl font-bold mt-1">{totalUsers}</h2>
            </div>
          ...
          <div className="bg-white rounded-2xl shadow-md p-6 flex items-center justify-between">
            <div>
              ...
              <h2 className="text-2xl font-bold mt-1 text-red-500">{inactiveUsers}</h2>
            </div>
          ...
          <div className="bg-white rounded-2xl shadow-md p-6 flex items-center justify-between">
            <div>
              ...
              <h2 className="text-2xl font-bold mt-1 text-[#67c6c6]">
                {
                  users.filter(
                    (u) =>
                      new Date() - new Date(u.joined) < 30 * 24 * 60 * 60 * 1000
                  ).length
                }
              </h2>
            </div>
